...
const { activateTab, getTabTitle, setTabTitle, closeTab, getTabURL, getTabContentWindow,
        getTabForBrowser, getTabReadyState,
...
const Tab = Class({
  ...
  initialize: function initialize(options) {
    ...
    tabInternals.opened = true;

    // emulate events for tabs that are already open
    emit(this, 'open', this);
    if (getTabReadyState(tab) === 'complete') {
      emit(this, 'ready', this);
    }

    // TabReady
    tab.browser.addEventListener(
        EVENTS.ready.dom,
        tabInternals.onReady = onTabReady.bind(this),
        false);

    // TabSelect
    window.BrowserApp.deck.addEventListener(
        EVENTS.activate.dom,
        tabInternals.onSelect = onTabSelect.bind(this),
        false);

    // TabClose
    window.BrowserApp.deck.addEventListener(
        EVENTS.close.dom,
        tabInternals.onClose = onTabClose.bind(this),
        false);
...
function cleanupTab(tab) {
  ...
  tabInternals.window.BrowserApp.deck.removeEventListener(EVENTS.activate.dom, tabInternals.onSelect, false);
  tabInternals.onSelect = null;
