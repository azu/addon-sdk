...
const { getOwnerWindow, getActiveTab, getTabs, getTabReadyState,
        openTab, getTabContentWindow } = require("../tabs/utils");
...
const WindowTabTracker = Trait.compose({
  ...
  _initWindowTabTracker: function _initWindowTabTracker() {
    ...
    for each (let tab in getTabs(this._window)) {
      ...
      this._onTabOpen(tab);
console.log('fire open!' + getTabReadyState(tab));
      if (getTabReadyState(tab) === 'complete') {console.log('fire ready!');
